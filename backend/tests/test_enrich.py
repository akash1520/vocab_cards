...
def test_enrich_word_returns_suggested_fields(authed_client: TestClient):
    with patch(
        ...
        response = authed_client.post("/api/words/enrich", json={"term": "ephemeral"})
...
def test_enrich_word_requires_auth(client: TestClient):
    response = client.post("/api/words/enrich", json={"term": "ephemeral"})
    assert response.status_code == 401


def test_enrich_word_returns_503_when_ollama_unreachable(authed_client: TestClient):
...
def test_enrich_word_returns_502_after_unparseable_json_retry(authed_client: TestClient):
    with patch(
        "app.services.ollama._call_ollama",
